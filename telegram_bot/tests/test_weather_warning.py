...
fake_warn_model_same = WarnModel("text", "type", "level")
fake_warn_model_diff = WarnModel("text_diff", "type", "level")
fake_chat = Chat()
...
def test_diff_warnings(mockit):
    ...
    # same warnings
    warning_mock.return_value = fake_warn_model_same
    asyncio.run(tasks.cron_send_warning(fake_chat, fake_ding_bot))
    message_mock.assert_called_once()

    # different warnings
    warning_mock.return_value = fake_warn_model_diff
    asyncio.run(tasks.cron_send_warning(fake_chat, fake_ding_bot))
